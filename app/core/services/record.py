...
async def make_comparison(request: Request, session: AsyncSession):
    ...
    try:
        ...
        if request.base_year:
            ...
            if request.race == 'white':
                ...
                wf_percent_change_pop = w_data.num_females_with_income / \
                    w_data1.num_females_with_income * 100
    ...
    try:
        ...
        if request.base_year:
            ...
            if request.race == "asian":
                ...
                af_percent_change_pop = a_data.num_females_with_income / \
                    a_data1.num_females_with_income * 100
    except NoResultFound:
    ...
    if request.base_year:
        ...
        m_percent_change_income = (wm_percent_change_income if request.race ==
                                   "white" else am_percent_change_income if request.race == "asian" else None)

        f_percent_change_income = (wf_percent_change_income if request.race ==
                                   "white" else af_percent_change_income if request.race == "asian" else None)

        m_percent_change_pop = (wm_percent_change_pop if request.race ==
                                "white" else am_percent_change_pop if request.race == "asian" else None)
        f_percent_change_pop = (wf_percent_change_pop if request.race ==
                                "white" else af_percent_change_pop if request.race == "asian" else None)
    ...
    f_comparison = Comparison({"income_difference": max(wf_income,
                                                        af_income) - min(wf_income,
                                                                         af_income),
                               "head": f_head,
                               "population_difference": max(w_data.num_females_with_income,
                                                            a_data.num_females_with_income) - min(w_data.num_females_with_income,
